...
public class ArmTrapezoidal extends Command {
  private Arm m_arm;
  ...
  public ArmTrapezoidal(Arm arm, double goal) {
    m_arm = arm;
    ...
    m_profile = new TrapezoidProfile(new TrapezoidProfile.Constraints(ArmConstants.kMaxSpeed, ArmConstants.kMaxAcceleration));
  ...
  public void initialize() {
    m_start = new TrapezoidProfile.State(m_arm.getEncoderPosition(), 0);
    m_timer.restart();
  ...
  public void execute() {
    ...
    double speed = m_pid.calculate(m_arm.getEncoderPosition(), idealState.position);
