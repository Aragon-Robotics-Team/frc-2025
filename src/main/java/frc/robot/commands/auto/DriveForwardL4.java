...
public class DriveForwardL4 extends ParallelCommandGroup {
  ...
  public DriveForwardL4(SwerveDrive swerve, Arm arm, Elevator elevator, EndEffector endEffector, Joystick secondJoystick) {
    ...
    addCommands(
      ...
      // new SequentialCommandGroup(new WaitCommand(5.5), (new SpinEndEffectorMotor(endEffector, -0.9, isScheduled(), secondJoystick)).withTimeout(1)) // 7s move for time so add an extra half second for buffer
